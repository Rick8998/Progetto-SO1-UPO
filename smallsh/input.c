...
/*Userin riempie inpbuf con la riga in input */
int userin(/*char *p,*/ char *name, char *home)		/* stampa il prompt e legge una riga */
{
...
/*Gettok copia un simbolo (riconoscendone la fine) da inbuf a tokbuf scorrendoli con
ptr e tok; il puntatore che gli è stato passato (&arg[narg]) punta all'inizio del
simbolo*/
int gettok(char **outptr)	/* legge un simbolo e lo mette in tokbuf */
{
  ...
  *tok++ = *ptr;
  ...
  switch(*ptr++){
    ...
    case '&': /*permetto il riconoscimento del carattere &*/
